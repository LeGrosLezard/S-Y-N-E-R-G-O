...
def mask_head(gray, img, copy, convexhull):
    ...
    return bitwise_and(copy, copy, mask=mask)
...
def concatenate_areas(area):
    ...
    numpy_horizontal_concat2 = np.concatenate( (area[6], area[7]), axis=1 )

    for i in range(2, 6):
        numpy_horizontal_concat = np.concatenate( (numpy_horizontal_concat, area[i]), axis=1 )

    for i in range(8, 12):
        numpy_horizontal_concat2 = np.concatenate( (numpy_horizontal_concat2,  area[i]), axis=1 )
...
def intra_face_displaying(img, gray, landmarks, face, fgbg, displaying):
    ...
    all_area = [cv2.resize(img[i[1]:i[1] + i[3], i[0]:i[0] + i[2]], (100, 50)) for i in area_by_contour + area_by_rectangle]

    areas = concatenate_areas(all_area)
...
def face_displaying(gray, img, convexhull, head_points, landmarks, fgbg):
    ...
    areas =  { "chin":[58, 56, 9, 7], "beet_eyes" :[21, 22, 27], "chin1":[58, 7, 3, 48],
               "chin2": [56, 54, 13, 9], "cheek1": [48, 3, 0, 28], "noze_area":[27, 48, 54],
               "cheek2":[54, 13, 16, 28], "mouse":(48, 61), "onEye1":(17, 22), "onEye2":(22, 27),
