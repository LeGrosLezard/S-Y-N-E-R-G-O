...
def writting_data(name_csv, liste_header, DICO):                    #Add into csv
...
def last_csv_into_folder():                                         #Recuperate last csv
    ...
    if liste_csv == []:                                             #Empty folder
##        path = PATH_OK_CSV + "/" + "1.csv"                          #Create csv
##        liste_csv = ["1.csv"]
        liste_csv = [None]
...
def transform_informations_to_data(informations, label):
    DICO = {"label": label,
...
def main_csv(informations):

    last_csv = last_csv_into_folder()               #Last csv name
    print(last_csv)

    if last_csv != None:
        create_new = verify_length_csv(last_csv)
    else:
        create_new = True
        last_csv = 0
    ...
    if create_new is True:
        ...
        label = "1"
        liste_header, DICO = transform_informations_to_data(informations, label)
        header_csv(csv_name, liste_header)
